...
# 			left=data[j][i]
# 			continue
# 		if left is not None and left=='T':
# 			print(j,i,'left')
# 			answer+=1
# 		if (j+i)%2 == 0:
# 			if j-1>=0:
...
# 		if data[y][x] == '#' or data[y][x]=='.': continue
...
# 			dy=1
#
# 		for (cost, x, y) in ((cost + 1, x + 1, y), (cost + 1, x - 1, y), (cost + 1, x, y + dy)):
...
while len(positions) > 0:
	...
	for (cost, x, y) in sorted(positions): #remove sorted here if it's not needed
		...
		if x==endX and y==endY:
			print(cost)
			sys.exit(0)
		if (x, y, cost%3) in costs and cost >= costs[(x, y, cost%3)]: continue #change >= here to > if you need to analyze ties
		costs[(x, y, cost%3)] = cost
		...
		for (cost, x, y) in ((cost + 1, x + 1, y), (cost + 1, x - 1, y), (cost + 1, x, y + dy), (cost + 1, x, y)):
			newPositions.append((cost, x, y))
	positions=newPositions

	newData = [['.' for i in range(W)] for j in range(H)]
	for j in range(H):
		for i in range(W):
			newJ,newI=(i-j)//2,W-1-(i+j)%2-j*2-(i-j)//2
			if newJ<0 or newJ>=H or newI<0 or newI>=W: continue
			# print(i,j,newI,newJ)
			# if data[newJ][newI]=='.': continue
			newData[newJ][newI] = data[j][i]
	data=newData
	endY,endX=(endX-endY)//2,W-1-(endX+endY)%2-endY*2-(endX-endY)//2
	# for l in data:
	# 	print(''.join(l))
