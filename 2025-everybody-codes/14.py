...
# 				total+=1 if data[y][x]=='#' else 0
#
# 			if data[j][i]=='#':
# 				newData[j][i]='#' if (total%2==1) else '.'
# 			else:
# 				newData[j][i]='#' if (total%2==0) else '.'
# 	data=newData
# 	answer+=sum(sum(1 if data[j][i]=='#' else 0 for i in range(H)) for j in range(H))
# print(answer)
centerData = data
visited={}
...
for n in range(2025000):
	...
	for j in range(H):
		for i in range(W):
			...
			if data[j][i]=='#':
				...
				newData[j][i]='#' if (total%2==0) else '.'
			v=1 if newData[j][i]=='#' else 0
			key=(key<<1)+v
			lit+=v
	good=True
	for j in range(H2):
		for i in range(W2):
			if newData[j+diffH][i+diffW]!=centerData[j][i]:
				good=False
				break
	...
	if key in visited:
		...
		break
	visited[key]=n
	if good: visitedSpecial.append((n, lit))
	# print(n, lit, len(visited))
	data=newData

cycles=1000000000//4095
partial=1000000000%4095
print(cycles, partial)

for (n, lit) in visitedSpecial:
	print(cycles, ' * ', lit)
	answer+=cycles*lit
	if n<=partial:
		print(lit)
