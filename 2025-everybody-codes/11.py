...
# 	if not changes: break
# 	n+=1
# 	# print(n, data, sum((i+1)*j for i, j in enumerate(data)))
# print(n)
...
averageBinSize=sum(data)//len(data)
maxDifference=0
#
# for i in range(len(data)-1):
# 	if data[i+1]>data[i]:
# 		difference=min(abs(averageBinSize-data[i]), abs(data[i+1]-averageBinSize))
# 		print(difference)
# 		maxDifference=max(maxDifference,difference)
# print(n, '@@@', maxDifference)
print(data)
for i in range(len(data)-1):
	if data[i+1]>=data[i]:
		difference=averageBinSize-data[i]
		if difference<0:
			print('oof', i, difference, averageBinSize, data[i], data)
			break
		data[i]+=difference
		data[i+1]-=difference
		maxDifference=max(maxDifference, difference)
