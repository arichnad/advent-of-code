...
# W=100;H=100
# data=data[0]
# m=[[False for i in range(W)] for j in range(H)]
# x,y=W//2,H//2
# startX,startY=x,y
#
# dir=0
# for line in data:
# 	ch,length=line[0],int(line[1:])
# 	if ch=='R':
# 		dir+=1
# 	else:
# 		dir-=1
...
# m[endY][endX]=False
# for mm in m:
# 	print(''.join(['#' if v else ' ' for v in mm]))
...
# 		if m[y][x]: continue
# 		if (x, y) in costs: continue #change >= here to > if you need to analyze ties
# 		costs[(x, y)] = cost
#
...
for line in data:
	...
	lines.append((x+first*dx,y+first*dy,x+dx*length,y+dy*length))
	first=False
	x,y=x+dx*length,y+dy*length
	endX,endY=x,y
lines[-1]=(lines[-1][0],lines[-1][1],lines[-1][2]-dx,lines[-1][3]-dy)
...
while len(positions) > 0:
	print(len(positions))
	newPositions = set()
	for (cost, x, y, last) in sorted(positions):
		if (x, y) in costs and cost >= costs[(x, y)][0]:
			continue
		costs[(x, y)] = (cost, last)
		...
		for dir in range(4):
			...
			for (x0,y0,x1,y1) in lines:
				if x0==x1 and (x0==x or sign(x0-x)==dx) and dy==0:
					y0,y1=min(y0,y1),max(y0,y1)
					t=abs(x0-x)
					t2=(endX-x)*dx
					for u in (t-1,t+1,t2):
						if 0 < u <= maxDistance: tryDistance.add(u)
					if y0<=y<=y1:
						maxDistance=min(maxDistance,t-1)
				if y0==y1 and (y0==y or sign(y0-y)==dy) and dx==0:
			...
			for u in tryDistance:
				if 0 < u <= maxDistance:
					# thisLast=list(last)
					# thisLast.append((x+dx*u, y+dy*u))
					thisLast=()
					newPositions.add((cost+u, x+dx*u, y+dy*u, thisLast))
	positions=newPositions
print()
print(costs[(endX, endY)])
