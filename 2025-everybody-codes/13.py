...
for [a,b] in data:
	...
	thing+=1
	totalLength += l
posPartial=202520252025%totalLength
while True:
	(start, length, rev) = d[pos]
	if posPartial<length:
		print(rev)
		print(start+posPartial if not rev else start+length-1-posPartial)
		break
	posPartial-=length
	pos=(pos+1)%len(d)
