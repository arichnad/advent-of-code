...
categories='byr iyr eyr hgt hcl ecl pid'.split(' ')
...
def valid(category, value):
	if category == 'byr':
		...
		return value.endswith('cm') and 150 <= int(value.replace('cm', '')) <= 193 or \
			value.endswith('in') and 59 <= int(value.replace('in', '')) <= 76
	elif category == 'hcl':
...
for line in data:
	...
	for thing in line.split(' '):
		category, value = thing.split(':')
		if category == 'cid': continue
		if valid(category, value):
			current.remove(category)
		else:
			current += 'invalid'
print(count)
