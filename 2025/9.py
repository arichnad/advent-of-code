...
xs,xLines=set(),[]
for i in range(H):
	[x0,y0],[x1,y1]=data[i],data[(i+1)%H]
	# xs.add(x0)
	xs.add(x0-1)
	xs.add(x0+1)
	if y0==y1:
		xLines.append((y0, min(x0, x1), max(x0, x1)))
...
for a in range(len(data)):
	for b in range(a+1,len(data)):
		[cx,cy],[dx,dy]=data[a],data[b]
		ex,fx=min(cx,dx),max(cx,dx)
		ey,fy=min(cy,dy),max(cy,dy)

		good=True
		for x in xs:
			if not (ex <= x <= fx): continue
			position=0
			count=0
			for (y0, x0, x1) in xLines:
				if not (x0 < x < x1): continue
				previousCount = count
				count += 1
				if position==0:
					if y0>=ey:
						ourCount = count if y0==ey else previousCount
						if ourCount%2==0:
							good=False
							break
						position=1
				if position==1:
					...
					if count%2==0:
						good=False
						break
			if position<2 or not good:
				good=False
				break
		if not good: continue
		cur=(abs(cx-dx)+1)*(abs(cy-dy)+1)
		if cur>m: m=cur
print(m)
